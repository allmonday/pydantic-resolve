...
> **整洁架构层次**：应用业务规则
>
> `post_*` 方法是应用业务规则层的第二部分。它们在数据组装完成后运行，计算企业业务规则层不需要知道的派生值。

`resolve_*` 加载缺失的数据。`post_*` 计算派生字段 — 从已组装的子树中推导出的值，如计数、汇总和格式化字符串。
...
那个时机就是为什么 `post_*` 非常适合派生字段 — 计数、汇总、格式化字符串以及其他从已解析子树中计算得出的值。
