...
- `config_resolver(er_diagram=diagram)`: 创建一个带有 ERD 的新自定义 Resolver 类
...
- `expose_as` (list[tuple[str, str]] | None): 通过 ExposeAs 暴露给后代节点的字段与别名对
- `send_to` (list[tuple[str, tuple[str, ...] | str]] | None): 通过 SendTo 发送到收集器的字段与目标对
